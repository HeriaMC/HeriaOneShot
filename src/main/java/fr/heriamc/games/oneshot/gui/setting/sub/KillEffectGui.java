...
public class KillEffectGui extends SubSettingGui<KillCosmetics> {
    ...
    protected ItemBuilder item(KillCosmetics cosmetic, int i, int i1) {
        ...
        return icon.setName(gamePlayer.hasSelected(CosmeticType.KILL_EFFECT, cosmetic) ? cosmetic.getName() + " §a[Sélectionner]" : cosmetic.getName())
                .setLoreWithList(
                        " ",
                ...
                .onClick(event -> {
                    if (cosmetic.has(gamePlayer) && !gamePlayer.hasSelected(CosmeticType.KILL_EFFECT, cosmetic)) {
